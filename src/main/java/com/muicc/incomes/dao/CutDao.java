...
public interface CutDao extends JpaRepository<Cut,Integer> {
    ...
    @Query(value = "select * from cut where cdid =?1",nativeQuery = true)
    List<Cut> getCutByCdid(int cdid);

    @Query(value = "select * from cut where eid = ?1 and cdid =?2",nativeQuery = true)
    List<Cut> getByEidAndCdid(int eid,int cdid);
    ...
    @Transactional
    @Modifying
    @Query(value = "insert into cut(ccid, cut, eid,cdid,status) values (?1,?2,?3,?4,1) ",nativeQuery = true)
